...
class Parser:
    def __init__(self, grammar, sentence, debug=False):
        ...
        self.charts = [Table([]) for i in range(len(self) + 1)]
    ...
    def init_first_chart(self):
        ...
        for rule in self.grammar.rules['S']:
            self.charts[0].add_row(Item(rule=rule, dot=0, start=0))
        # row = ChartRow(Rule(Parser.GAMMA_SYMBOL, ['S']), 0, 0)
        # self.charts[0].add_row(row)

    #Thêm item có dạng pro -> I, (current - 1)
    def prescan(self, chart, position):
        ...
        if word:
            ...
            for rule in rules:
                chart.add_row(Item(rule, 1, position - 1))
            ...
            if row.is_complete():
                ...
                for r in self.charts[row.start].rows:
                    if completed == r.next_category():
                        new = Item(r.rule, r.dot + 1, r.start, r, [row])
                        chart.add_row(new)
